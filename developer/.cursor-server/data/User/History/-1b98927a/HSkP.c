...
void assign_to_clusters(double **datapoints, int N, double **centroids, int K, int dimension, int *assignments) {
    ...
    for (i = 0; i < N; i++) {
        double min_distance = euclidean_distance(datapoints[i], centroids[0], dimension);
        int closest_cluster = 0;
        
        for (k = 1; k < K; k++) {
            double distance = euclidean_distance(datapoints[i], centroids[k], dimension);
...
int has_converged(double **old_centroids, double **new_centroids, int K, int dimension) {
    ...
    for (k = 0; k < K; k++) {
        double distance = euclidean_distance(old_centroids[k], new_centroids[k], dimension);
        if (distance >= EPSILON) {
            return 0;
        }
    }
...
int main(int argc, char *argv[]) {
    int K, max_iter = 400, N, dimension, iter, i, d;
