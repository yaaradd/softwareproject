...
export class BrowserManager {
	...
	async getContext(): Promise<Context> {
		...
		if (this.config.connectionType === 'self') {
			...
			// Create a directory for this session's persistent context
			// Use workspace-scoped storage if provided, otherwise fall back to temp directory
			if (this.storageDir) {
				this.persistentContextDir = this.storageDir;
				PlaywrightLogger.info(
					`Using workspace-scoped persistent context directory: ${this.persistentContextDir}`
				);
			} else {
	...
	async dispose(): Promise<void> {
		...
		// Clean up persistent context directory if it was a temporary directory
		// Don't delete workspace-scoped storage directories as they should persist
		if (this.persistentContextDir && !this.storageDir) {
			try {
				PlaywrightLogger.info(
					`Cleaning up temporary persistent context directory: ${this.persistentContextDir}`
				);
				await fs.rm(this.persistentContextDir, {
					recursive: true,
					force: true,
				});
				PlaywrightLogger.info('Temporary persistent context directory cleaned up');
			} catch (error) {
				PlaywrightLogger.error(
					'Error cleaning up temporary persistent context directory',
					error as Error
				);
			}
			this.persistentContextDir = null;
		} else if (this.persistentContextDir) {
			PlaywrightLogger.info(
				`Preserving workspace-scoped persistent context directory: ${this.persistentContextDir}`
			);
			this.persistentContextDir = null;
		}
	}
