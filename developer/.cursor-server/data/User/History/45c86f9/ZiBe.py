...
def run_test_case(name, input_data, args, expected_rc, expected_snippet=None, check_py=True):
    log(f"ðŸ§ª Test: {name}", Colors.HEADER)
    ...
    if expected_rc == 0 and c_pass and py_pass and check_py:
        ...
        if c_clean != py_clean:
            log(f"   âš ï¸  WARNING: C and Python outputs differ!", Colors.WARNING)
...
def load_file(filename):
    ...
    try:
        ...
        print(f"âš ï¸  WARNING: '{filename}' not found. Test will be skipped or fail.")
        return None
...
if __name__ == "__main__":
    ...
    log("\nðŸš€ Starting The ULTIMATE K-means Tester ðŸš€\n", Colors.BOLD)
    ...
    if passed_tests == total_tests:
        ...
        log("âš ï¸  Some tests failed. Check logs above.", Colors.WARNING)
    log("="*40, Colors.BOLD)